<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:79:"D:\PHPTutorial\WWW\open\public/../application/exchange\view\exchange\index.html";i:1530847244;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<script type="text/javascript">
    ...
    function exchangeDel(id){
        layer.confirm('确认删除此订单?', {icon: 3, title:'提示'}, function(index){
            //do something
            $.getJSON('./exchangeDel', {'id' : id}, function(res){
                if(res.code == 0){
                    layer.alert('删除成功');
                    initTable();
                }else{
    ...
    function exchangeSave(id){
        layer.confirm('确认充值?', {icon: 3, title:'提示'}, function(index){
            //do something
            $.getJSON('./exchangeSave', {'id' : id}, function(res){
                if(res.code == 0){
                    layer.alert('更新成功');
                    initTable();
                }else{
